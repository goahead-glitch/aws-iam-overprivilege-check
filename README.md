...
AWS 계정의 IAM 과권한을 읽기 전용으로 점검하고, 실제 사용 이력을 근거로 최소권한 정책을 역산한 프로젝트다. 앞서 인프라를 비교했던 4인 팀([when-nodes-run-out](https://github.com/goahead-glitch/when-nodes-run-out))과 같은 구성으로 시작했지만, 설계와 실행은 혼자 진행했다. (2026년 6월, 2주간 수행)
